...
public class TileController : MonoBehaviour
{
    ...
    {
        if (render.sprite == null || board.IsAnimating || game.IsGameOver) return;
        ...
        else
        {
            ...
            else
            {
                if (GetAllAdjacentTiles().Contains(previousSelected))
                {
                    ...
                    SwapTile(otherTile, () => {
                        if (board.GetAllMatches().Count > 0) board.Process();
                        else
                        {
    ...
    private TileController GetAdjacent(Vector2 castDir)
    {
        ...
        if (hit) return hit.collider.GetComponent<TileController>();
    ...
    private List<TileController> GetOneLineMatch(Vector2[] paths)
    {
        ...
        if (matchingTiles.Count >= 2) return matchingTiles;
    ...
    public List<TileController> GetAllMatches()
    {
        ...
        if (verticalMatchingTiles != null) matchingTiles.AddRange(verticalMatchingTiles);
