...
public class BoardManager : MonoBehaviour
{
    ...
    public static BoardManager Instance
    {
        get
        {
            if (_instance == null)
            {
                ...
                if (_instance == null) Debug.LogError("Fatal Error: BoardManager not Found");
            }
            return _instance;
        }
    }
    ...
    private List<TileController> GetAllDestroyed()
    {
        ...
        for (int x = 0; x < size.x; x++)
        {
            for (int y = 0; y < size.y; y++)
            {
                if (tiles[x, y].IsDestroyed) destroyedTiles.Add(tiles[x, y]);
            }
        }
    ...
    private IEnumerator RepositionTiles(System.Action onCompleted)
    {
        ...
        for (int x = 0; x < size.x; x++)
        {
            for (int y = 0; y < size.y; y++)
            {
                ...
                if ((Vector2)tiles[x, y].transform.position == targetPosition) continue;
